...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper" ng-app="DetalleAPP" ng-controller="DetalleControlador">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header"> Informe del Traslado |
                        <a href="<?php url("traslado/index")?>" class="btn btn-default">
...
$origen_nombre = $ori->Ejecutar('sp_buscar_destino', array($traslado->id_origen, $_SESSION['cliente']));
...
$destino_nombre = $des->Ejecutar('sp_buscar_destino', array($traslado->id_destino, $_SESSION['cliente']));
...
$estado_nombre = $est->Ejecutar('sp_buscar_estado', array($traslado->id_sub_estado, $_SESSION['cliente']));
                            ...
                            <table class="table table-hover">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php
if ($traslado->progreso == 1) {
    echo "Pendiente";
} elseif ($traslado->progreso == 2) {
    echo "Completo";
}
?>
                                        </td>
...
foreach ($productos as $producto) {
    //Verificamos que el origen y destino
    if ($destino_nombre[0]['nombre'] == $origen_nombre[0]['nombre']) {
        echo "Se cambia el estado de " . $producto['cantidad'] . " " . $producto['nomcat'] . " (" .
            $producto['cate'] . ") Modelo " . $producto['model'] . " a " . $estado_nombre[0]['nombre'] . "<br>";
        $prod = new libreria\ORM\EtORM();
        $series = $prod->ejecutar("sp_series_informe", array($producto['id'], $traslado_id));
        foreach ($series as $serie) {
            echo $serie['serie'] . "<br>";
        }
        echo "<br>";
    } else {
        echo "Se cambia de stock de " . $producto['cantidad'] . " " . $producto['nomcat'] . " (" .
            $producto['cate'] . ") Modelo " . $producto['model'] . " de " . $origen_nombre[0]['nombre'] .
            " a " . $destino_nombre[0]['nombre'] . "<br>";
        $prod = new libreria\ORM\EtORM();
        $series = $prod->ejecutar("sp_series_informe", array($producto['id'], $traslado_id));
        foreach ($series as $serie) {
            echo $serie['serie'] . "<br>";
...
?>
                ...
                <br>
                <?php echo nl2br($traslado->comentario) ?>
                <br>
                <br>
            </div>
            ...
            <div class="modal fade" id="cambiarRemito" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                <div class="modal-dialog" id="remito" role="document">
                    <div class="modal-content">
                        <form action="<?php url("traslado/remito/" . $traslado->id)?>" method="post">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <input type="text" class="form-control" required name="remito" id="remito"
                                                value="<?php echo $traslado->remito ?>" />
                                        </div>
                                    </div>
            ...
            <div class="modal fade" id="cambiarFecha" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                <div class="modal-dialog" id="fecha" role="document">
                    <div class="modal-content">
                        <form action="<?php url("traslado/fecha/" . $traslado->id)?>" method="post">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                        aria-hidden="true">&times;</span></button>
                                    ...
                                    <div class="col-lg-12">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" id="fecha" name="fecha" type="date"
                                                value="<?php echo isset($traslado) ? $traslado->fecha : date("Y-m-d") ?>">
                                        </div>
                                    </div>
            ...
            <div class="modal fade" id="cambiarOrigen" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                <div class="modal-dialog" id="origen" role="document">
                    <div class="modal-content">
                        <form action="<?php url("traslado/origen/" . $traslado->id)?>" method="post">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                        aria-hidden="true">&times;</span></button>
...
$prove = $pro->ejecutar("sp_entrada_origen", array($_SESSION['cliente'], "cliente"));
foreach ($prove as $provedor) {?>
                                                <option <?php echo isset($traslado) && $traslado->id_origen == $provedor['id']
    ? 'selected' : '' ?> value="<?php echo $provedor['id'] ?>">
            ...
            <div class="modal fade" id="cambiarDestino" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                <div class="modal-dialog" id="destino" role="document">
                    <div class="modal-content">
                        <form action="<?php url("traslado/cambiarDestino/" . $traslado->id)?>" method="post">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                        aria-hidden="true">&times;</span></button>
...
$prove = $pro->ejecutar("sp_entrada_origen", array($_SESSION['cliente'], "cliente"));
foreach ($prove as $provedor) {?>
                                                <option <?php echo isset($traslado) && $traslado->id_destino == $provedor['id']
    ? 'selected' : '' ?> value="<?php echo $provedor['id'] ?>">
                                                    <?php echo ($provedor['nombre']); ?>
                                                </option><?php }?>
                                            </select>
            ...
            <div class="modal fade" id="cambiarEstado" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                <div class="modal-dialog" id="estado" role="document">
                    <div class="modal-content">
                        <form action="<?php url("traslado/cambiarEstado/" . $traslado->id)?>" method="post">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                        aria-hidden="true">&times;</span></button>
...
foreach ($sub_estado as $sub_estados) {?>
                                                <option <?php echo isset($traslado) && $traslado->id_sub_estado == $sub_estados['id']
    ? 'selected' : '' ?> value="<?php echo $sub_estados['id'] ?>">
            ...
            <div class="modal fade" id="cambiarObservacion" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                <div class="modal-dialog" id="observacion" role="document">
                    <div class="modal-content">
                        <form action="<?php url("traslado/cambiarObservacion/" . $traslado->id)?>" method="post">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                        aria-hidden="true">&times;</span></button>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <input type="text" class="form-control" required name="observacion"
                                                id="observacion" value="<?php echo $traslado->observacion ?>" />
                                        </div>
                                    </div>
            ...
            <div class="modal fade" id="cambiarComentario" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                <div class="modal-dialog" id="comentario" role="document">
                    <div class="modal-content">
                        <form action="<?php url("traslado/cambiarComentario/" . $traslado->id)?>" method="post">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                        aria-hidden="true">&times;</span></button>
                            ...
                            <div class="modal-body">
                                <div class="row">
                                    ...
                                    <div class="col-lg-12">
                                        <div class="form-group">
                                            <textarea class="form-control" rows="5" name="comentario" id="comentario"><?php echo $traslado->comentario ?>
                                        </textarea>
                                        </div>
                                    </div>
            ...
            <div class="modal fade" id="cambiarTicket" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                <div class="modal-dialog" id="ticket" role="document">
                    <div class="modal-content">
                        <form action="<?php url("traslado/cambiarTicket/" . $traslado->id)?>" method="post">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                        aria-hidden="true">&times;</span></button>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <input type="text" class="form-control" required name="ticket" id="ticket"
                                                value="<?php echo $traslado->ticket ?>" />
                                        </div>
                                    </div>
            ...
            <div class="modal fade" id="cambiarEstadoFinal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                <div class="modal-dialog" id="estadoFinal" role="document">
                    <div class="modal-content">
                        <form action="<?php url("traslado/cambiarEstadoFinal/" . $traslado->id)?>" method="post">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                        aria-hidden="true">&times;</span></button>
...
foreach ($subEstados as $estado) {?>
                                                <option <?php echo isset($traslado) && $traslado->id_sub_estado == $estado['id']
...
foreach ($progresos as $progre) {?>
                                                <option <?php echo isset($traslado) && $traslado->progreso == $progre['id']
    ? 'selected' : '' ?> value="<?php echo $progre['id'] ?>">
